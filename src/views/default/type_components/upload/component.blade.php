<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        @if($value)
			...
			$is_s3 = stripos($value, 'amazonaws.com') !== false;
			$s3_status = false;
			if($is_s3){
				$parsed = parse_url($value);
				$s3_status = Storage::disk('s3')->exists($parsed['path']);
			}

			if($s3_status || Storage::exists($value) || file_exists($value)):
			$url = asset($value);
            ...
            @if(!$readonly || !$disabled)
                ...
                <p><a class='btn btn-danger btn-delete btn-sm' onclick="if(!confirm('{{trans("crudbooster.delete_title_confirm")}}')) return false"
                      href='{{url(CRUDBooster::mainpath("delete-image?image=".$value."&id=".$row->{$primary_key}."&column=".$name))}}'><i
                                class='fa fa-ban'></i> {{trans('crudbooster.text_delete')}} </a></p>
            @endif
        @endif
        @if(!$value)
            <input type='file' id="{{$name}}" title="{{$form['label']}}" data-validation="{{$required}}" {{$readonly}} {{$disabled}} class='form-control' name="{{$name}}"/>

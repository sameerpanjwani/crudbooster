...
class CBController extends Controller
{
	...
	public function getIndex()
	{
		...
		foreach ($columns_table as $index => $coltab) {
			...
			$field = @$coltab['name'];
			$join_column_user = @$coltab['join_column'];
			...
			$result->where(function ($w) use ($filter_column, $fc) {
				foreach ($filter_column as $key => $fc) {
					...
					if (strpos($type, 'custom') !== false) {
						$type_custom = explode('|_|', $type);
						$type = isset($type_custom[0]) ? $type_custom[0] : '';
						$value = isset($type_custom[1]) ? $type_custom[1] : '';
						$operator = isset($type_custom[2]) ? $type_custom[2] : '';
					}
		...
		foreach ($data['result'] as $row) {
			...
			foreach ($columns_table as $col) {
				...
				if (isset($col['image'])) {
					if ($value == '') {
						$value = "<a  data-lightbox='roadtrip' rel='group_{{$table}}' title='$label: $title' href='".asset('vendor/crudbooster/image no')."'><img width='40px' height='40px' src='".asset('vendor/crudbooster/product_image_not_available.png')."'/></a>";
					} else {
